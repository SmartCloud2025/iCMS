 ...
 */
var imageUploader = {},imageManagerUrl = null,
    flashObj = null,
    postConfig=[];
(function () {
    var g = $G,
    ...
    imageUploader.refreshDir = function(){
    	loadimageManager(imageManagerUrl);
    	return false;
    }
    imageUploader.getDir = function(url){
    	loadimageManager(url);
    	return false;
    }
    imageUploader.insertImage = function(url){
    	var imgObj = {};
        imgObj.src = url;
        imgObj._src = url;
    	insertImage(imgObj);
        ...
        dialog.onok = function () {
            ...
            switch (currentTab) {
                ...
                case "local":
                    ...
                    break;
                case "imgManager":
                    return insertChecked("imageList");
                    break;
                case "imgSearch":
    ...
    function insertChecked(id) {
        var inputs = $G(id).getElementsByTagName("input"), imgObjs = [];
        for (var i = 0, ci; ci = inputs[i++];) {
            if (ci.checked) {
                var url = ci.value.replace(/(\s*$)/g, ""), img = {};
                img.src = url;
                img._src = url;
                imgObjs.push(img);
            }
	...
	function loadimageManager(url){
		imageManagerUrl	= url||editor.options.imageManagerUrl;
        ajax.request(imageManagerUrl, {
            timeout:100000,
            //action:"get",
            ...
            onerror:function () {
                imageList.html(lang.imageLoadError);
            }
        });
	}
    /**
