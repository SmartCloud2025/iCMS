$(function(){
  var register = $("#iCMS_register");
  $(".iCMS_registerBtn").click(function(){
    ...
    $("input[id^='iCMS_']",register).each(function(){
    ...
    $.post("<!--{$iURL.api.user}-->",param,function(ret){
      if(ret.code){
        ...
        var a = document.getElementById('iCMS_'+ret.forward);
        msg(a,ret.msg);
      }
    },'json');

  })
  $("input[id^='iCMS_']",register).click(function(){
    msg(this);
  }).blur(function(){
    iCMS_check_form(this);
  });
...
function msg(e,c){
  ...
  var n = e.name,err  = $(".err_"+n).css('visibility','visible');
...
function ajax_check(a){
  ...
  if(typeof(val)===undefined ||val==""||val!=a.value){
    ...
    $.get("<!--{$iURL.api.check}-->",{name:a.name,value:a.value},
      function(c){
        //a.name = c.forward
        if(c.code){
          msg(a,"ok");
