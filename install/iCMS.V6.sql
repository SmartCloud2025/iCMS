...
CREATE TABLE `#iCMS@__article` (
  ...
  `tags` varchar(5120) NOT NULL DEFAULT '' COMMENT '标签',
  ...
  `top` smallint(6) unsigned NOT NULL DEFAULT '0' COMMENT '权重',
  ...
  PRIMARY KEY (`id`),
  KEY `idx_hits` (`status`,`hits`),
  KEY `idx_status` (`status`,`id`),
  KEY `idx_pubdate` (`status`,`pubdate`),
  KEY `idx_order` (`status`,`ordernum`),
  KEY `idx_top_order` (`status`,`top`,`ordernum`),
  KEY `idx_userid` (`status`,`userid`,`id`),
  KEY `idx_userid_hits` (`status`,`userid`,`hits`),
  KEY `idx_cid` (`status`,`cid`,`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
insert  into `#iCMS@__prop`(`pid`,`rootid`,`cid`,`field`,`type`,`ordernum`,`name`,`val`) values (1,0,0,'pid','article',0,'头条','1'),(2,0,0,'pid','article',0,'首页推荐','2'),(3,0,0,'pid','category',0,'推荐栏目','1'),(4,0,0,'pid','tags',0,'热门标签','1');
