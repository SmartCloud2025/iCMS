...
* @version 6.0.0
* @$Id: job.app.php 634 2013-04-03 06:02:53Z coolmoo $
*/
class JOB{
    ...
    function tkreport($userid,$ym){
	    ...
	    for ($i=1; $i<=$totalday; $i++) {
			$TBAPI->set_method('taobao.taobaoke.report.get');
			$TBAPI->set_param('fields',"trade_id,pay_time,pay_price,num_iid,outer_code,commission_rate,commission,seller_nick,pay_time,app_key"); 
			if($i<10){
				$TBAPI->set_param('date',"{$y}{$m}0".$i); 
			}else{
				$TBAPI->set_param('date',"{$y}{$m}".$i); 
			}
			$taobaokeRs = $TBAPI->execute();
			$_report	= $taobaokeRs->taobaoke_report->taobaoke_report_members->taobaoke_report_member;
			if($_report){
				foreach($_report AS $row){
					$this->commission[$row->outer_code][]=$row->commission;
					$this->report[$row->outer_code]=$row;
				}
			}
	    }
    ...
    function countPost($userid){
		...
		foreach((array)$rs AS $key=>$a){
			$this->dayCountPost[date('Y-m-j',$a['postime'])]++;
			if($a['status']!="1"){
				$this->count0post++;
			}
			if($a['status']=="1"){
				$this->countTimePost($a['postime'],$this->today['start'],$this->today['end'],$this->today['count']);
				$this->countTimePost($a['postime'],$this->yesterday['start'],$this->yesterday['end'],$this->yesterday['count']);
				$this->countTimePost($a['postime'],$this->month['start'],$this->month['end'],$this->month['count']);
				$this->countTimePost($a['postime'],$this->pmonth['start'],$this->pmonth['end'],$this->pmonth['count']);
			}else{
				$this->countTimePost($a['postime'],$this->today['start'],$this->today['end'],$this->today['count0']);
			}
		}
    }
	function countTimePost($t,$s,$e,&$c){
		if($t>=$s && $t<=$e){
			$c++;
		}
	}
